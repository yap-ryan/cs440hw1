...
public class HillClimbingAgent extends Agent {
     ...
     * @param xExtent: the max X coordinate of the map
    		...
    		int xExtent, int yExtent, Set<Coordinate> obstacles)
    {
    	...
    	int curX = pt.getXPosition();
    	int curY = pt.getYPosition();
    	
    	Coordinate upLeft = new Coordinate(curX-1,curY-1);
    	Coordinate up = new Coordinate(curX,curY-1);
    	Coordinate upRight = new Coordinate(curX+1,curY-1);
    	Coordinate left = new Coordinate(curX-1,curY);
    	Coordinate right = new Coordinate(curX+1,curY);
    	Coordinate downLeft = new Coordinate(curX-1,curY+1);
    	Coordinate down = new Coordinate(curX,curY+1);
    	Coordinate downRight = new Coordinate(curX+1,curY+1);
    	
    	Set<Coordinate> childStates = new HashSet<Coordinate>();
    	childStates.add(upLeft);
    	childStates.add(up);
    	childStates.add(upRight);
    	childStates.add(left);
    	childStates.add(right);
    	childStates.add(downLeft);
    	childStates.add(down);
    	childStates.add(downRight);
    	
    	// Filter out invalid child states
    	for (Coordinate c : childStates) {
    		
    		// Footman outside map
    		
    		for (Coordinate obs : obstacles) {	
    			// Footman inside obstacle! Remove child state
    			if (c.equals(obs)) {
    				childStates.remove(c);
    			}
    		}
