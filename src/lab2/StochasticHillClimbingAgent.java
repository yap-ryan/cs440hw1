...
public class StochasticHillClimbingAgent extends Agent {
	...
	/**
	 * A method that converts a list of N utilities into a list of N probabilities. This method
	 * assumes that lower utilities are better, so it converts smaller utility values into 
	 * larger probabilities. This is done with the following equation:
	 *  prob[position i] = (1.0 / utility[position i]) / (sum from j=1 -> N of (1.0 / utility[position j]))
	...
	/**
	 * A method for picking a child at random where the probability of a child is also specified.
	 * This is done by first argsorting the probabilities in descending order.
	 * We then conver the probabilities (which are a pmf) into a cmf in descending order.
	 * By choosing a random number, we can incorporate the "weight" of each child by finding
	 * the position j in the cmf where the random value is greater than the cmf value.
	 * We can then use that position j to look up the corresponding position in the argsort
	 * and finally the child that corresponds to that argsorted position.
